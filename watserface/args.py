...
from watserface.filesystem import get_file_name, is_image, is_video, resolve_file_paths
from watserface.jobs import job_store
from watserface.normalizer import normalize_fps, normalize_padding
from watserface.processors.core import get_processors_modules
from watserface.types import ApplyStateItem, Args
from watserface.vision import create_image_resolutions, create_video_resolutions, detect_image_resolution, detect_video_fps, detect_video_resolution, pack_resolution
...
def apply_args(args : Args, apply_state_item : ApplyStateItem) -> None:
	...
	# identity profiles
	apply_state_item('identity_profile_id', None)
	# modeler
	apply_state_item('source_profile_id_for_modeler', None)
	apply_state_item('modeler_target_path', None)
	apply_state_item('modeler_model_name', None)
	apply_state_item('modeler_epochs', 100)
	apply_state_item('lora_rank', 16)
	apply_state_item('lora_learning_rate', 0.0001)
	apply_state_item('modeler_batch_size', 4)
	# context-aware blending
	apply_state_item('context_blending_enabled', True)
	apply_state_item('blend_method', 'poisson')
	apply_state_item('blend_strength', 0.8)
	# instantid refinement
	apply_state_item('instantid_post_swap_enabled', False)
	apply_state_item('instantid_post_swap_strength', 0.8)
	apply_state_item('instantid_post_blend_enabled', False)
	apply_state_item('instantid_post_blend_strength', 0.6)
	apply_state_item('instantid_boundary_enabled', False)
	apply_state_item('instantid_boundary_strength', 0.5)
	# execution
