...
import watserface.uis.overrides as uis_overrides
from watserface import logger, metadata, state_manager, wording
from watserface.exit_helper import hard_exit
from watserface.filesystem import resolve_relative_path
from watserface.uis.types import Component, ComponentName
...
def load_ui_layout_module(ui_layout : str) -> Any:
	try:
		ui_layout_module = importlib.import_module('watserface.uis.layouts.' + ui_layout)
...
def launch() -> None:
	from watserface.uis.components import header
	...
	with gradio.Blocks(theme = get_theme(), css = get_css(), title = metadata.get('name') + ' ' + metadata.get('version'), fill_width = True) as ui:
		# Render header first (logo + system settings)
		header.render()
		header.listen()
