...
from watserface.types import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
...
from watserface.uis.ui_helper import convert_str_none
from watserface.vision import normalize_frame_color, read_static_image, read_video_frame
...
def render() -> None:
	...
	with gradio.Group():
		...
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or watserface.choices.face_selector_age_range[0]
			...
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				...
				step = calc_int_step(watserface.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		...
		maximum = watserface.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
