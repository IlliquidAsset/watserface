...
from watserface.common_helper import calc_int_step
from watserface.ffmpeg import get_available_encoder_set
from watserface.filesystem import is_image, is_video
from watserface.types import AudioEncoder, Fps, VideoEncoder, VideoPreset
from watserface.uis.core import get_ui_components, register_ui_component
...
def render() -> None:
	...
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		...
		maximum = watserface.choices.output_image_quality_range[-1],
	...
	OUTPUT_AUDIO_QUALITY_SLIDER = gradio.Slider(
		...
		maximum = watserface.choices.output_audio_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_AUDIO_VOLUME_SLIDER = gradio.Slider(
		...
		maximum = watserface.choices.output_audio_volume_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		...
		maximum = watserface.choices.output_video_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
