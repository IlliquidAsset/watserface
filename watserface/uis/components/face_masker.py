...
def update_face_occluder_choices() -> gradio.Dropdown:
	...
	if trained_model_file_paths:
		for model_file_path in trained_model_file_paths:
			...
			if model_name not in watserface.choices.face_occluder_models:
				if 'xseg' in model_name.lower() or 'occlusion' in model_name.lower() or 'mask' in model_name.lower():
					watserface.choices.face_occluder_models.append(model_name) #type:ignore

	return gradio.Dropdown(choices = watserface.choices.face_occluder_models)
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider]:
	face_mask_types = face_mask_types or watserface.choices.face_mask_types
...
def update_face_mask_areas(face_mask_areas : List[FaceMaskArea]) -> gradio.CheckboxGroup:
	face_mask_areas = face_mask_areas or watserface.choices.face_mask_areas
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or watserface.choices.face_mask_regions
