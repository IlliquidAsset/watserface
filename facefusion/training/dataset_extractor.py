...
from facefusion.vision import read_video_frame, count_video_frame_total, read_image, write_image
...
def extract_training_dataset(
	...
	Args:
		...
		frame_interval: Skip every N frames
		max_frames: Maximum frames to extract per video
		progress: Gradio progress object
		
	Returns:
		Dict with statistics {'frames_extracted': int, 'landmarks_saved': int}
	"""
	stats = {'frames_extracted': 0, 'landmarks_saved': 0}
	
	for source_path in source_paths:
		if is_video(source_path):
			_extract_video(source_path, output_dir, frame_interval, max_frames, stats, progress)
		else:
			_extract_image(source_path, output_dir, stats, progress)
